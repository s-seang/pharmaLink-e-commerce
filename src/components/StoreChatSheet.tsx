...
export function StoreChatSheet({
  ...
  const keep = (blob: Blob, kind: AttachKind, name: string, length?: number) => {
  ...
  const addFiles = (files: FileList | File[] | null | undefined) => {
    ...
    chosen.forEach((file) => keep(file, kindOf(file.type), file.name || 'Attachment'))
    setAttaching(false)
  ...
  const finishRecording = (keepIt: boolean) => {
    ...
    media.onstop = () => {
      if (keepIt && chunks.current.length > 0) {
        keep(new Blob(chunks.current, { type: media.mimeType }), 'audio', 'Voice message', held)
      }
      chunks.current = []
    }
   ...
   */
  const placeholder = `Message ${store.name}…`.replace(/ /g, ' ')
                    ...
                    {carried.length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {carried.map((item) => (
                          ...
                          <SentAttachment key={item.id} item={item} tight={carried.length > 1} />
                        ))}
                      </div>
                    )}

                    {message.text && (
                      <p className={`whitespace-pre-wrap ${carried.length > 0 ? 'mt-2' : ''}`}>
                        {message.text}
                      </p>
                    )}
        ...
        >
          ...
          {attaching && (
            <div className="menu-in absolute bottom-full left-3 z-20 mb-2 w-52 overflow-hidden rounded-xl border border-line bg-white shadow-xl">
              <AttachOption icon={Image} label="Photo" onClick={() => pick(photoPicker)} />
              <AttachOption icon={Video} label="Video" onClick={() => pick(videoPicker)} />
              <AttachOption
                icon={Camera}
                label="Camera"
                onClick={() => {
              ...
              />
              <AttachOption icon={FileText} label="File" onClick={() => pick(filePicker)} />
            </div>
          )}
            ...
            <div>
              <div className="no-scrollbar mb-2 flex gap-2 overflow-x-auto pb-1 pt-0.5">
                {attachments.map((item) => (
                  <DraftAttachment key={item.id} item={item} onRemove={() => remove(item.id)} />
                ))}
              </div>
            </div>
          </div>
      ...
      {camera && (
        <CameraCapture
          ...
          onCapture={(blob, shotKind) =>
            keep(blob, shotKind, shotKind === 'video' ? 'Camera video' : 'Camera photo')
          }
...
/**
 * A recording waiting in the composer. It plays where it sits, so a voice
 * message can be heard back before it is sent — or removed and done again.
 */
function VoiceDraft({ item }: { item: Attachment }) {
  ...
  return (
    <button
      ...
      className="flex h-16 w-36 items-center gap-2.5 rounded-lg border border-line bg-surface px-2.5 text-left lg:h-20 lg:w-44"
    >
      ...
      <span className="min-w-0">
        <span className="flex items-center gap-1 text-[11px] font-medium text-ink">
          <Mic size={12} className="shrink-0 text-teal" />
          Voice
        </span>
        {item.seconds !== undefined && (
          <span className="block text-[10px] text-muted">{clock(item.seconds)}</span>
        )}
      </span>
...
function DraftAttachment({ item, onRemove }: { item: Attachment; onRemove: () => void }) {
  return (
    <div
    ...
    >
      ...
      {item.kind === 'audio' && <VoiceDraft item={item} />}
...
function SentAttachment({ item, tight }: { item: Attachment; tight?: boolean }) {
  ...
  if (item.kind === 'image') {
    return <img src={item.url} alt={item.name} className={`rounded-lg object-cover ${frame}`} />
  }
  ...
  if (item.kind === 'audio') {
    return (
      <div>
        ...
        <audio controls src={item.url} className="mt-1.5 h-9 w-48 max-w-full lg:w-64">
          <track kind="captions" />
        </audio>
      </div>
    )
  ...
  return (
    ...
    >
      ...
      <span className="max-w-[12rem] truncate lg:max-w-[18rem]">{item.name}</span>
    </a>
  )
}
